...
export class DataBaseService {
      ...
      this.leerUsuario('avalenzuela@duocuc.cl').then(async usuario => {
        ...
        this.leerUsuario('cfuentes@duocuc.cl').then(async usuario => {
          ...
          this.leerUsuario('admin@duocuc.cl').then(async usuario => {
            if (!usuario) await this.guardarUsuario(Usuario.getUsuario('admin@duocuc.cl', 'admin', 'admin', 'admin', 'Dónde nació la mamá del admin', 'admin'));
        });
      });
    });
  });
  ...
  async validarUsuarioPorCorreo(correo: string): Promise<Usuario | undefined> {
